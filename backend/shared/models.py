"""
Shared Pydantic v2 data models for 3D Model Pipeline.

This module defines all core entities used across model-converter and slicer components.
All models provide runtime validation, type safety, and JSON serialization.

Feature: 002-3d-model-pipeline
Spec: /specs/002-3d-model-pipeline/data-model.md
"""
...
class QualityMetrics(BaseModel):
    ...
    def from_raw_metrics(
        ...
        overall = (
            0.25 * ssim
            + 0.20 * (1.0 - (lpips or 0.2))  # LPIPS lower is better
            + 0.20 * edge_iou
            + 0.15 * color_corr
            + 0.10 * coverage
...
class VectorFile(BaseModel):
    ...
    @classmethod
    def validate_overall(cls, v: bool, info):  # type: ignore
        """Overall validation requires all structure checks to pass."""
        values = info.data
        return (
            values.get("is_valid_xml", False)
            and values.get("has_root_element", False)
            and values.get("has_viewbox", False)
            and values.get("has_geometry", False)
        )
...
class MeshFile(BaseModel):
    ...
    @classmethod
    def validate_printability(cls, v: bool, info):  # type: ignore
        """Printable requires watertight AND manifold AND fits build volume."""
        values = info.data
        return (
            values.get("is_watertight", False)
            and values.get("is_manifold", False)
            and values.get("fits_build_volume", False)
            and not values.get("face_count_reject", True)
        )

    @field_validator("depth_accuracy_pct")
    @classmethod
    def calculate_depth_accuracy(cls, v: float, info):  # type: ignore
...
class GCodeFile(BaseModel):
    ...
    @field_validator("is_valid")
    @classmethod
    def validate_gcode(cls, v: bool, info):  # type: ignore
        """Valid G-code must be non-empty and contain temperature commands."""
        values = info.data
        return values.get("file_size_bytes", 0) > 0 and values.get("has_temperature_commands", False)
